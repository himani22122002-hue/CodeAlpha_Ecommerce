...
export default function Checkout() {
  ...
  const handlePlaceOrder = (e) => {
    e.preventDefault();
    if (!/^\d{10}$/.test(formData.phone)) return alert("Invalid Phone Number (10 digits required)");
    if (!/^\d{6}$/.test(formData.pinCode)) return alert("Invalid PIN Code (6 digits required)");
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <form onSubmit={handlePlaceOrder} className="grid lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit">
          ...
          <button type="submit" className="w-full mt-6 bg-orange-500 text-white py-3 rounded-xl font-semibold hover:bg-orange-600">Place Order</button>
        </div>
      </form>
    </div>
  );
